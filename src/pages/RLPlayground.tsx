...
const DIR = [[-1, 0], [0, 1], [1, 0], [0, -1]]; // row, col deltas
const ARROWS = ["↑", "→", "↓", "←"];
...
const RLPlayground = () => {
  ...
  const qTableRef = useRef<number[][][]>([]); // [row][col][action]
  ...
  const initWorld = useCallback(() => {
    ...
    // Walls
    const wallPositions = [
      [2, 2], [2, 3], [2, 4], [3, 4], [5, 1], [5, 2],
      [7, 5], [7, 6], [7, 7], [4, 8], [4, 9], [8, 3],
      [1, 6], [9, 7], [9, 8],
    ];
    for (const [wr, wc] of wallPositions) {
      if (wr < GRID && wc < GRID) world[wr][wc] = { reward: 0, wall: true, terminal: false };
    }
    // Small positive reward near goal
  ...
  const stepAgent = useCallback(() => {
    ...
    if (Math.random() < epsilon) {
      ...
      const qValues = Q[agent.row][agent.col];
    ...
    // Bounds & wall check
    if (nr < 0 || nr >= GRID || nc < 0 || nc >= GRID || world[nr][nc].wall) {
      nr = agent.row;
      nc = agent.col;
    }
    ...
    // Q-learning update
    Q[agent.row][agent.col][action] +=
      lr * (reward + gamma * maxNextQ - Q[agent.row][agent.col][action]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      for (let i = 0; i < speed; i++) stepAgent();
    }, 16);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Grid dimensions
      const gridArea = Math.min(w * 0.55, h * 0.85);
      const cellSize = gridArea / GRID;
      const offsetX = 30;
      const offsetY = (h - gridArea) / 2;
      ...
      for (let r = 0; r < GRID; r++) {
        for (let c = 0; c < GRID; c++) {
          ...
          if (cell.wall) {
            ...
            ctx.fillStyle = "hsl(150, 70%, 20%)";
          } else if (cell.terminal && cell.reward < 0) {
            ctx.fillStyle = "hsl(0, 50%, 18%)";
          } else {
            // Color by max Q value
            if (showValues && Q[r] && Q[r][c]) {
              const maxQ = Math.max(...Q[r][c]);
              const minQ = Math.min(...Q[r][c]);
              const range = Math.max(0.1, Math.abs(maxQ));
              const norm = Math.max(-1, Math.min(1, maxQ / (range + 5)));
              if (norm > 0) {
                ctx.fillStyle = `hsla(175, 70%, ${10 + norm * 20}%, 1)`;
              } else {
                ctx.fillStyle = `hsla(0, 50%, ${10 + Math.abs(norm) * 15}%, 1)`;
              }
            } else {
          ...
          // Terminal labels
          if (cell.terminal) {
            ctx.fillStyle = cell.reward > 0 ? "hsl(150, 70%, 55%)" : "hsl(0, 70%, 55%)";
            ctx.font = `${Math.max(10, cellSize * 0.3)}px 'JetBrains Mono'`;
          ...
          // Policy arrows
          if (showPolicy && !cell.wall && !cell.terminal && Q[r] && Q[r][c]) {
            const qValues = Q[r][c];
            const maxQ = Math.max(...qValues);
            if (maxQ !== 0) {
              const bestAction = qValues.indexOf(maxQ);
              ctx.fillStyle = "hsla(175, 80%, 50%, 0.7)";
              ctx.font = `${Math.max(10, cellSize * 0.35)}px sans-serif`;
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";
              ctx.fillText(ARROWS[bestAction], px + cellSize / 2, py + cellSize / 2);
            }
          }

          // Value text
          if (showValues && !cell.wall && !cell.terminal && Q[r] && Q[r][c]) {
            const maxQ = Math.max(...Q[r][c]);
            if (Math.abs(maxQ) > 0.01) {
              ctx.fillStyle = "hsla(215, 15%, 55%, 0.6)";
              ctx.font = `${Math.max(7, cellSize * 0.15)}px 'JetBrains Mono'`;
              ctx.textAlign = "center";
              ctx.textBaseline = "bottom";
              ctx.fillText(maxQ.toFixed(1), px + cellSize / 2, py + cellSize - 3);
            }
          }
        }
      }
      ...
      // Agent
      const ax = offsetX + agent.col * cellSize + cellSize / 2;
      const ay = offsetY + agent.row * cellSize + cellSize / 2;
      ctx.fillStyle = "hsl(40, 90%, 55%)";
      ctx.beginPath();
      ctx.arc(ax, ay, cellSize * 0.25, 0, Math.PI * 2);
      ctx.fill();
      ...
      // Start marker
      const sx = offsetX + cellSize / 2;
      const sy = offsetY + (GRID - 1) * cellSize + cellSize / 2;
      ...
      // Learning curve (right side)
      const chartX = offsetX + gridArea + 40;
      const chartY = offsetY;
      const chartW = w - chartX - 20;
      const chartH = gridArea * 0.5;

      if (chartW > 80) {
        ctx.fillStyle = "hsla(220, 18%, 9%, 0.9)";
        ctx.fillRect(chartX - 5, chartY - 5, chartW + 10, chartH + 30);
        ctx.strokeStyle = "hsl(220, 15%, 18%)";
        ctx.lineWidth = 1;
        ctx.strokeRect(chartX - 5, chartY - 5, chartW + 10, chartH + 30);

        ctx.fillStyle = "hsl(215, 15%, 50%)";
        ctx.font = "10px 'JetBrains Mono'";
        ctx.textAlign = "left";
        ctx.textBaseline = "top";
        ctx.fillText("EPISODE REWARD", chartX, chartY + 3);
        ...
        if (hist.length > 1) {
          ...
          for (let i = 0; i < hist.length; i++) {
            ...
            const y = chartY + chartH / 2 - (hist[i] / maxR) * (chartH / 2) * 0.8 + 10;
          ...
          if (hist.length > 10) {
            ...
            ctx.beginPath();
            const wSize = 10;
            for (let i = wSize; i < hist.length; i++) {
              const avg = hist.slice(i - wSize, i).reduce((a, b) => a + b, 0) / wSize;
              const x = chartX + (i / (hist.length - 1)) * chartW;
              const y = chartY + chartH / 2 - (avg / maxR) * (chartH / 2) * 0.8 + 10;
              if (i === wSize) ctx.moveTo(x, y); else ctx.lineTo(x, y);
            }
            ctx.stroke();
          }
          ...
          ctx.setLineDash([3, 3]);
          ctx.beginPath();
          ctx.moveTo(chartX, chartY + chartH / 2 + 10);
          ctx.lineTo(chartX + chartW, chartY + chartH / 2 + 10);
          ctx.stroke();
          ctx.setLineDash([]);
        }
    ...
    <>
      ...
      <ControlSection title="Display">
        <div className="flex gap-2">
          <button onClick={() => setShowValues(!showValues)}
            className={`flex-1 text-xs font-mono py-2 rounded border transition-colors ${showValues ? "border-primary text-primary" : "border-border text-foreground"}`}>
            Q-Values
          </button>
          <button onClick={() => setShowPolicy(!showPolicy)}
            className={`flex-1 text-xs font-mono py-2 rounded border transition-colors ${showPolicy ? "border-primary text-primary" : "border-border text-foreground"}`}>
            Policy
          </button>
        </div>
      </ControlSection>
      <ControlSection title="Simulation">
        <div className="flex gap-2">
          <button onClick={() => setRunning(!running)}
            className={`flex-1 text-xs font-mono py-2 rounded border transition-colors ${running ? "border-primary text-primary" : "border-border text-foreground hover:border-primary/50"}`}>
            {running ? "⏸ Pause" : "▶ Train"}
          </button>
          <button onClick={() => { initWorld(); setRunning(false); }}
      ...
      <ControlSection title="Guide">
        <p className="text-xs text-muted-foreground leading-relaxed">
          A <span className="text-primary">Q-learning</span> agent navigates a grid world to reach the <span className="text-green-glow">★ goal</span> while 
          avoiding <span className="text-red-glow">✕ pits</span>. Policy arrows show the greedy action, cell colors show Q-values. 
          The <span className="text-amber-glow">amber agent</span> learns through trial and error. Watch the learning curve converge as episodes increase.
        </p>
      </ControlSection>
    </>
  );
